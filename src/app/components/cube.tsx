...
function BoxPart({ position, color, degrees }: IBoxPart) {
  ...
  const rectangular = degrees
    ? polarToRectangular(
        initial_polar.r,
        initial_polar.theta + degToRad(degrees)
      )
    : null;
  const actual_position: [x: number, y: number, z: number] = rectangular
    ? [rectangular.x, rectangular.y, 1]
    : position;
  const actual_rotation = new THREE.Euler(
    0,
    0,
    degrees ? THREE.MathUtils.degToRad(degrees) : 0
  );

  return (
    <mesh position={actual_position} rotation={actual_rotation} ref={meshRef}>
      <boxGeometry />
